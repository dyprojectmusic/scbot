...
const token = '7197172509:AAHo_P7j2yVNOUO4gHOGdFta2XTJKB36hGM';
...
bot.onText(/\/createuser (.+) (.+)/, (msg, match) => {
  const userId = msg.from.id; // Get user ID
  if (userId === 6689252152) { // Check if the user is admin
    const username = match[1];
    const password = match[2];
    const url = `https://infogame-epep.000webhostapp.com/process.php?username=${username}&password=${password}&create`;
...
bot.onText(/\/start/, (msg) => {
  const chatId = msg.chat.id; 
  bot.sendMessage(chatId, "🔥 *BERIKUT MENU BOT DDOS fromhsj* 🔥\n\n" +
    ...
    "OWNER IS HERE",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '🔥CONTACH🔥', url: 'https://t.me/dycoders' }
          ]
        ]
      },
      parse_mode: "Markdown"
  ...
  bot.sendMessage(chatId, "CHENEL SAYA ADA DI BAWAH👇",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '❗ CHENEL ❗', url: 'https://t.me/testiunbanban' }
          ]
        ]
      },
      parse_mode: "Markdown"
...
bot.onText(/\/donasi/, (msg) => {
    const saweriaLink = 'https://saweria.co/dycoders';
    const buttonOptions = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: 'Donasi ke Saya', url: 'https://saweria.co/dycoders' }
                ]
            ]
        }
    };
